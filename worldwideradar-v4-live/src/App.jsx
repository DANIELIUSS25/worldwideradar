...
function useRSS() {
  ...
  const go = useCallback(async () => {
    const out = [];
    const KW = ["war","attack","conflict","military","killed","strike","missile","bomb","troops","ceasefire","offensive","invasion","airstrike","drone","battle","coup","artillery","shelling"];
    for (const feed of RSS_FEEDS) {
      try {
        const r = await fetch(`https://api.rss2json.com/v1/api.json?rss_url=${encodeURIComponent(feed.url)}&count=12`);
        const d = await r.json();
        if (d.items) d.items.forEach(item => {
          const txt = ((item.title||"")+" "+(item.description||"")).toLowerCase();
          if (KW.some(k => txt.includes(k)))
            out.push({ id:item.guid||item.link, title:item.title, link:item.link, date:new Date(item.pubDate), src:feed });
        });
      } catch(e) {}
    }
    out.sort((a,b)=>b.date-a.date);
    setSignals(out); setLoading(false);
  }, []);
  useEffect(() => { go(); const t = setInterval(go, REFRESH_MS); return ()=>clearInterval(t); }, [go]);
